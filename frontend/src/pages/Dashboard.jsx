    ...
    <div className="min-h-screen bg-[#0B0F17]">
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          ...
          <StatBig label="Kalite / Sevk" value={counts.kalite_kontrol + counts.sevk_alaninda} icon={FileText} accent="text-cyan-400" />
            ...
            {STATUSES.map((s) => {
              ...
              return (
                <div key={s.key} data-testid={`kanban-col-${s.key}`}
                  className={`w-80 flex-shrink-0 bg-[#131926] border border-[#2A364F] rounded-xl flex flex-col snap-start border-t-2 ${s.col}`}>
                  ...
                  <div className="p-3 space-y-3 flex-1 min-h-[120px]">
                    {items.map((p) => <PartCard key={p.id} part={p} onClick={setDetail} />)}
                    {items.length === 0 && <p className="text-center text-slate-600 text-xs py-6">Boş</p>}
                  </div>
                </div>
              );
            })}
